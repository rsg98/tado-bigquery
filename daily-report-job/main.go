...
func writeDailyReportsToBq(ctx context.Context, data [][]byte) error {
	...
	m := &tadodailyreport.Tadodailyreport{}
...
func parseZoneReportToProto(zoneReports []TadoZoneDetailsReport) ([][]byte, error) {
	...
	for n, z := range zoneReports {
		...
		settingsList := make([]*protos.Tadodailyreport_Settings, len(settingsData))
		for i := 0; i < len(settingsData); i++ {
			settingsList[i] = &protos.Tadodailyreport_Settings{
		...
		weatherList := make([]*protos.Tadodailyreport_Weather, len(weatherData))
		for i := 0; i < len(weatherData); i++ {
			weatherList[i] = &protos.Tadodailyreport_Weather{
		...
		callheatList := make([]*protos.Tadodailyreport_Callforheat, len(callheatData))
		for i := 0; i < len(callheatData); i++ {
			callheatList[i] = &protos.Tadodailyreport_Callforheat{
		...
		humidityList := make([]*protos.Tadodailyreport_Measureddata_Humidity, len(humidityData))
		for i := 0; i < len(humidityData); i++ {
			humidityList[i] = &protos.Tadodailyreport_Measureddata_Humidity{
		...
		insidetempList := make([]*protos.Tadodailyreport_Measureddata_Insidetemperature, len(insidetempData))
		for i := 0; i < len(insidetempData); i++ {
			insidetempList[i] = &protos.Tadodailyreport_Measureddata_Insidetemperature{
				Timestamp:   proto.Int64(insidetempData[i].Timestamp.Unix()),
				Temperature: proto.Float32(float32(insidetempData[i].Value.Celsius)),
			}
		}

		measuredList := make([]*protos.Tadodailyreport_Measureddata, 1)
		measuredList[0] = &protos.Tadodailyreport_Measureddata{
			InsideTemperature: insidetempList,
			Humidity:          humidityList,
		}

		reportInterval := &protos.Tadodailyreport_Interval{
			From: proto.Int64(z.Report.Interval.From.Unix()),
			To:   proto.Int64(z.Report.Interval.To.Unix()),
		}

		dayReportRow := &protos.Tadodailyreport{
